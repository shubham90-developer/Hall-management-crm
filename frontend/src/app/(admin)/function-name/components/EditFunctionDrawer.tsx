...
import { useGetAllFunctionByIdQuery, useUpdateFunctionTypeMutation } from '@/store/functionType'
import { toast } from 'react-toastify'

const EditFunctionDrawer = ({ item }: any) => {
  ...
  // api
  const { data: functionData } = useGetAllFunctionByIdQuery(item._id)
  const [updateFunction, { isLoading: updateLoading }] = useUpdateFunctionTypeMutation()
    ...
    <>
      ...
      <div
        ...
        <div className="p-3 border-bottom bg-light d-flex justify-content-between align-items-center">
          <h5 className="mb-0 fw-bold"> Edit Function</h5>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="p-3 border-top d-flex gap-2">
            ...
            <button className="btn btn-primary w-100" type="submit" disabled={updateLoading}>
              <IconifyIcon icon="solar:diskette-broken" /> {updateLoading ? 'Updating...' : 'Update'}
            </button>
          </div>
        </form>
      </div>
