...
import { useCreateExternalItemsMutation } from '@/store/externalItemsApi'
import { useGetAllBuggetNameQuery } from '@/store/buffetNameApi'

const AddExternalDrawer = ({ onAdd }: any) => {
  ...
  const [createExternalItems, { isLoading }] = useCreateExternalItemsMutation()
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    try {
      await createExternalItems(formData as any).unwrap()
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="p-3 border-top d-flex gap-2">
            ...
            <button className="btn btn-primary w-100" type="submit" disabled={isLoading}>
              <IconifyIcon icon="solar:diskette-broken" /> {isLoading ? 'Creating...' : 'Create Item'}
            </button>
          </div>
        </form>
      </div>
...
export default AddExternalDrawer
