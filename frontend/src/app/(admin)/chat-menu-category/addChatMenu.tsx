...
import { useCreateChatMenuCategoryMutation } from '@/store/chatMenuCategoryApi'

const AddChatMenuCategoryDrawer = ({ onAdd }: any) => {
  ...
  // api
  const [createChatMenuCategory, { isLoading }] = useCreateChatMenuCategoryMutation()
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    try {
      await createChatMenuCategory(formData as any).unwrap()
      setFormData({
        categoryName: '',
        status: 'Active',
      })
...
export default AddChatMenuCategoryDrawer
