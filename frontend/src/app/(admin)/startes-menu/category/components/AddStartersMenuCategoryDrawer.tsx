...
import { useCreateStartersMenuCategoryMutation } from '@/store/startersMenuCategory'

const AddStartersMenuCategoryDrawer = ({ onAdd }: any) => {
  ...
  // api
  const [createStartersMenuCategory, { isLoading }] = useCreateStartersMenuCategoryMutation()
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    try {
      await createStartersMenuCategory(formData as any).unwrap()
      setFormData({
        categoryName: '',
        status: 'Active',
      })
...
export default AddStartersMenuCategoryDrawer
