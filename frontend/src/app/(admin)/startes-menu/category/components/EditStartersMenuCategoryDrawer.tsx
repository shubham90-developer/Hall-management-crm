...
import { useGetStartersMenuCategoryByIdQuery, useUpdateStartersMenuCategoryMutation } from '@/store/startersMenuCategory'
import { toast } from 'react-toastify'

const EditStartersMenuCategoryDrawer = ({ item }: any) => {
  ...
  // api
  const { data: startersCategoryData, isLoading } = useGetStartersMenuCategoryByIdQuery(item._id)
  const [updateStartersCategory] = useUpdateStartersMenuCategoryMutation()

  useEffect(() => {
    if (startersCategoryData) {
      setFormData({
        categoryName: startersCategoryData?.categoryName || '',
        status: startersCategoryData?.status || 'Active',
      })
    }
  }, [startersCategoryData])
...
export default EditStartersMenuCategoryDrawer
