...
import { useGetsweetMenuByIdQuery, useUpdatesweetMenuMutation } from '@/store/sweetMenuApi'
import { useGetAllBuggetNameQuery } from '@/store/buffetNameApi'
import Select from 'react-select'

const EditSweetMenuDrawer = ({ item }: any) => {
  ...
  const { data: sweetMenuData, isLoading } = useGetsweetMenuByIdQuery(item._id)
  const [updateSweetMenu] = useUpdatesweetMenuMutation()

  const buffetOptions = buffetName.map((item: any) => ({ value: item._id, label: item.buffetName }))

  useEffect(() => {
    if (sweetMenuData) {
      const data = sweetMenuData as any
      setFormData({
    ...
    <>
      ...
      <div
        ...
        <div className="p-3 border-bottom bg-light d-flex justify-content-between align-items-center">
          <h5 className="mb-0 fw-bold"> Edit Sweet menu Item </h5>
          ...
          <div className="p-3">
            <div className="mb-3">
              ...
              <Select
                ...
                placeholder="🍽️ Choose Your Menu"
...
export default EditSweetMenuDrawer
