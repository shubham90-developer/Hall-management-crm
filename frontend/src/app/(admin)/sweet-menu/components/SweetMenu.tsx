...
import AddSweetMenuDrawer from './AddSweetMenuDrawer'
import EditSweetMenuDrawer from './EditSweetMenuDrawer'
import { toast } from 'react-toastify'
import Swal from 'sweetalert2'
import { ISweetMenu, useDeletesweetMenuMutation, useGetAllsweetMenuListQuery } from '@/store/sweetMenuApi'
...
const SweetMenu = () => {
  ...
  const { data: otherListData = [], isLoading, isError } = useGetAllsweetMenuListQuery()

  const [deleteOtherList] = useDeletesweetMenuMutation()

  // search
  const searchFunction = otherListData.filter((item: ISweetMenu) => {
      ...
      <Col xl={12}>
        <Card>
          <div className="d-flex card-header justify-content-between align-items-center">
            <CardTitle as="h4"> Sweet Menu List</CardTitle>
              ...
              <table className="table table-hover table-bordered align-middle mb-0">
                ...
                <tbody>
                  {currentItems.length > 0 ? (
                    currentItems.map((item: ISweetMenu, index: number) => (
                      <tr key={item._id}>
                        <td>{startIndex + index + 1}</td>
                        <td>
                          {Array.isArray(item.buffetName)
                            ? item.buffetName.map((b: any) => b.buffetName).join(', ')
                            : (item.buffetName as any)?.buffetName}
                        </td>
                        <td>{item.itemName}</td>
                        <td>₹ {item.price}</td>
