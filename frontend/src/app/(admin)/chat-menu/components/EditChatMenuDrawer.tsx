...
import { useGetChatMenuByIdQuery, useUpdateChatMenuMutation } from '@/store/chatMenuApi'
import { useGetAllChatMenuCategoryQuery } from '@/store/chatMenuCategoryApi'
import Select from 'react-select'

const EditChatMenuDrawer = ({ item }: any) => {
  ...
  // api
  const { data: chatMenu, isLoading } = useGetChatMenuByIdQuery(item._id)
  const [updateChatMenu] = useUpdateChatMenuMutation()
  const { data: categoryList = [] } = useGetAllChatMenuCategoryQuery()

  const categoryOptions = categoryList.map((cat) => ({ value: cat._id, label: cat.categoryName }))

  useEffect(() => {
    if (chatMenu) {
      setFormData({
        categoryName: (chatMenu as any).categoryName?._id || (chatMenu as any).categoryName || '',
        itemName: chatMenu.itemName,
        price: chatMenu.price,
        status: chatMenu.status,
      })
    }
  }, [chatMenu])
    ...
    <>
      ...
      <div
        ...
        <div className="p-3 border-bottom bg-light d-flex justify-content-between align-items-center">
          <h5 className="mb-0 fw-bold"> Edit Chat menu Item </h5>
...
export default EditChatMenuDrawer
