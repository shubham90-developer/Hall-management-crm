  ...
  const dateGroups = useMemo(() => {
    ...
    bookings
      ...
      .forEach((b: any) => {
        ...
        const dateKey = new Date(b.functionDate).toISOString().slice(0, 10)
                      ...
                      <tr key={group.date}>
                        ...
                        <td className="text-nowrap">
                          <div className="d-flex flex-wrap gap-2">
                            <Link href={`/grocery/${group.date}`} className="btn btn-soft-primary btn-sm" title="View Grocery Sheet">
                              <IconifyIcon icon="solar:eye-broken" className="fs-16 me-1" />
                              View
                            </Link>
                            <Link
                              href={`/grocery/${group.date}?autodownload=true`}
                              className="btn btn-soft-danger btn-sm"
                              title="Download Grocery PDF">
