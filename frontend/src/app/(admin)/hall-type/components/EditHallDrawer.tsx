...
import { useGetHallTypeByIdQuery, useUpdateHallTypeMutation } from '@/store/hallTypeApi'

const EditHallDrawer = ({ item }: any) => {
  ...
  // api
  const { data: hallTypeData, isLoading } = useGetHallTypeByIdQuery(item._id)
  const [updateHallType] = useUpdateHallTypeMutation()

  useEffect(() => {
    if (hallTypeData) {
      setFormData({
        hallName: hallTypeData?.hallName || '',
        status: hallTypeData?.status || 'Active',
      })
    }
  }, [hallTypeData])
    ...
    <>
      ...
      <div
        ...
        <div className="p-3 border-bottom bg-light d-flex justify-content-between align-items-center">
          <h5 className="mb-0 fw-bold"> Edit Hall</h5>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="p-3 border-top d-flex gap-2">
            ...
            <button className="btn btn-primary w-100" disabled={isLoading}>
              <IconifyIcon icon="solar:diskette-broken" /> {isLoading ? 'Adding...' : 'Update Hall'}
            </button>
          </div>
        </form>
      </div>
