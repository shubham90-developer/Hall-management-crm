...
import AddOtherDrawer from './AddOtherDrawer'
import EditOtherDrawer from './EditOtherDrawer'
import { toast } from 'react-toastify'
import Swal from 'sweetalert2'

import { useDeleteOtherListMutation, useGetAllOtherListQuery, type IOtherList } from '@/store/otherListApi'
...
const OtherList = () => {
  ...
  const { data: otherListData = [], isLoading, isError } = useGetAllOtherListQuery()

  const [deleteOtherList] = useDeleteOtherListMutation()

  // search
  const searchFunction = otherListData.filter((item: IOtherList) => {
  ...
  const handleDelete = async (id: string) => {
    const result = await Swal.fire({
      title: 'Are you sure?',
      text: 'You want to delete this Other Item?',
      icon: 'warning',
      ...
      <Col xl={12}>
        <Card>
          <div className="d-flex card-header justify-content-between align-items-center">
            <CardTitle as="h4">Other Items List</CardTitle>
              ...
              <table className="table table-hover table-bordered align-middle mb-0">
                ...
                <tbody>
                  {currentItems.length > 0 ? (
                    currentItems.map((item: IOtherList, index: number) => (
                      <tr key={item._id}>
