...
import { useGetOtherListByIdQuery, useUpdateOtherListMutation } from '@/store/otherListApi'
import { toast } from 'react-toastify'

const EditOtherDrawer = ({ item }: any) => {
  ...
  // api
  const { data: otherListData, isLoading } = useGetOtherListByIdQuery(item._id)
  const [updateOtherList, { isLoading: updateLoading }] = useUpdateOtherListMutation()

  // set data
  useEffect(() => {
    if (otherListData) {
      setFormData({
        itemName: otherListData.itemName || '',
        price: otherListData.price || '',
        status: otherListData.status || 'Active',
      })
    }
  }, [otherListData])
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    try {
      await updateOtherList({
        id: item._id,
        data: formData as any,
      }).unwrap()
      toast.success('Other List updated successfully')
      setOpen(false)
    } catch (error) {
...
export default EditOtherDrawer
